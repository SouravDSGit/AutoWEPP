...
Python scripts for automated batch processing and execution of the Water Erosion Prediction Project (WEPP) hillslope model.

## Overview

This repository contains Python tools to streamline WEPP model workflows, including climate data preprocessing, batch simulation execution, output parsing, and result aggregation. The scripts handle multiple hillslope scenarios automatically, reducing manual processing time and potential errors.

## Features

- **Climate Data Processing**: Calculates daily precipitation metrics, storm duration, and intensity patterns from sub-daily data
- **Automated Model Execution**: Batch runs WEPP simulations for multiple hillslope configurations
- **Output Processing**: Extracts and formats summary data from WEPP output files
- **Data Aggregation**: Combines results across simulations into analysis-ready datasets
- **Configurable Parameters**: Easy modification of file paths, hillslope parameters, and output formats

## Scripts

### 1. `precipitation_processor.py`

Processes hourly and sub-hourly precipitation data to generate daily summaries with storm characteristics. Calculates:

- Total daily precipitation depth
- Storm duration (hours with non-zero precipitation)
- Time to peak (tp) and intensity ratio (ip) for each storm event
- Handles both historical (1980-2020) and recent (2021-2023) datasets

### 2. `wepp_batch_runner.py`

Automates WEPP executable runs for multiple hillslope scenarios. Generates run files with appropriate inputs and captures execution logs.

### 3. `summary_parser.py`

Parses WEPP summary output files and converts them to CSV format. Adds hillslope area and representative width data to outputs.

### 4. `data_aggregator.py`

Combines individual simulation results into a single output file with key metrics including precipitation, runoff, sediment yield, and total erosion.
...
- pandas
- numpy
- WEPP model executable

## Usage

1. **Preprocess Climate Data**: Run `precipitation_processor.py` to prepare daily precipitation inputs
2. Configure file paths in each script to match your directory structure
3. Ensure WEPP input files (.slp, .cli, .sol, .man) are properly formatted and located
4. **Run Model Simulations**: Execute `wepp_batch_runner.py`
5. **Parse Outputs**: Run `summary_parser.py`
6. **Aggregate Results**: Run `data_aggregator.py`
...
Mukherjee, Sourav, Sudhanshu Panda, Devendra M. Amatya, Mariana Dobre, John L. Campbell, Roger Lew, Peter Caldwell, Kelly Elder, Johnny M. Grace, and Sherri L. Johnson. "Hydro-geomorphological assessment of culvert vulnerability to flood-induced soil erosion using an ensemble modeling approach." Environmental Modelling & Software 183 (2025): 106243.

Sourav Mukherjee. (2024). Automated Data Pipeline for Batch Pre-processing, Post-processing, and Running Water Erosion Prediction Project (WEPP) Model Simulation Using Python (Version 1.0) [Software]. https://github.com/SouravDSGit/WEPP_model_run
